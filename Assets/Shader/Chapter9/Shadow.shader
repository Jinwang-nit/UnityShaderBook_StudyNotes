    ...
    SubShader
    {
        Pass
        {
            ...
            struct v2f
            {
                ...
                SHADOW_COORDS(2) // 声明一个用于对阴影采样的纹理坐标，参数是下一个可用的插值寄存器索引
            };
            ...
            float4 frag(v2f v) : SV_Target
            {
                ...
                float3 shadow = SHADOW_ATTENUATION(v);

                float atten = 1.0; // 光照衰减，平行光是没有衰减所以就直接让他等于1.0就行了
                return float4(ambient + (specular + diffuse) * atten * shadow, 1.0);
            }
            ENDCG
        }
